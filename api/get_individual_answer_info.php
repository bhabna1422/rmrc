...
function User_patient_info($data){
    ...
    if ($validation->fails()) {
        ...
        $rows=[];
        $result = db_query("SELECT `interview_date`,`visit_number`, `district_id`, `block_id`, `village_id`,`contact_number`,`sample_id`, `survey_type` FROM `answer_individual_schedule`
		WHERE sample_id = '$sample_id'");  
        // $data1 = mysqli_fetch_array($result);
		// echo $data1['village_id'];
        if(mysqli_num_rows($result) > 0){
            ...
            $response['message'] = "Individual Answer Info Found";
